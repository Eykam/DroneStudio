...
Own champion file /workspace/t4_dag8b_best.json - t4_best untouched.
"""
...
def main():
    ...
    for k in range(1, ROUNDS + 1):
        ...
        json.dump(flat, open(f"/workspace/t4_dag8b_r{k}.json", "w"))
        res = eval_all27(flat, f"t4-dag8b r{k}")
    ...
    if best["round"] > 0:
        import shutil
        shutil.copy(f"/workspace/t4_dag8b_r{best['round']}.json", "/workspace/t4_dag8b_best.json")
        # v1-plant regression read on the best (SoC pins to 1.0 on v1)
        os.environ.pop("AUTORESEARCH_MOTOR_V2", None)
        res1 = eval_all27(json.load(open("/workspace/t4_dag8b_best.json")), "t4-dag8b-best-v1plant")
        os.environ["AUTORESEARCH_MOTOR_V2"] = "1"
        m1 = float(np.mean(list(res1.values())))
        print("T4DAG8M2V4_BEST_SAVED t4_dag8b_r%d mean_m2=%.3f v1_regression=%s mean=%.3f" % (
            best["round"], best["mean"],
            json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res1.items()}), m1), flush=True)
    P.post_status({"training": {"status": "idle", "name": "t4_dagger8b",
