...
Checkpoints {"champion": flat, "R1": ..., "R2": ...} -> t4_ppo6b_uN.json,
best-with-floors -> t4_ppo6b_best.json. Series "t4-ppo6b uN" per cell +
t4_ppo6b_return / t4_ppo6b_face. t4_best.json never touched.
"""
...
def main():
    ...
    res0 = eval_all_res(pack, "t4-ppo6b u0")
    print("T4PPO6B u0 (residual=0 == champion): " +
          json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res0.items()}), flush=True)
    best_mean = -1e9
    json.dump(pack, open("/workspace/t4_ppo6b_u0.json", "w"))
    floor_fails = 0
    ...
    for u in range(1, UPDATES + 1):
        ...
        pack = {"champion": flat, "R1": R1.tolist(), "R2": R2.tolist()}
        json.dump(pack, open(f"/workspace/t4_ppo6b_u{u}.json", "w"))
        res = eval_all_res(pack, f"t4-ppo6b u{u}")
        ...
        if floors_ok and mean > best_mean:
            best_mean = mean
            json.dump(pack, open("/workspace/t4_ppo6b_best.json", "w"))
        P.post_series("t4_ppo6b_return", mean_ret, f"u{u}")
        P.post_series("t4_ppo6b_face", mean_face, f"u{u}")
        if floor_fails > 8:
            print(f"T4PPO6B ABORT u{u}: floors failed 8 consecutive updates "
                  f"(precision collapse guard)", flush=True)
            break
        resid_norm = float(np.abs(R2).max())
        print(f"T4PPO6B u{u}: ret={mean_ret:.2f} face={mean_face:.3f} " +
        ...
        P.post_status({
            "training": {"status": "running", "name": "t4_ppo6b_residual", "iter": u, "iters": UPDATES,
    ...
    P.post_status({"training": {"status": "idle", "name": "t4_ppo6b_residual",
                                "note": f"residual RL pilot finished; best mean {best_mean:.3f}"}})
    print(f"T4PPO6B_DONE best_mean={best_mean:.3f}", flush=True)
