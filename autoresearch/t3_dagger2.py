"""DAgger-T3 v2: fix the two v1 failure causes.

1. Anchor dilution: anchor samples are now 50% of EVERY batch (v1 used a
   fixed 2048/batch while the DAgger set grew to 39k -> anchor weight
   decayed to nothing; T0-T2 regressed).
2. Retrain drift: each round retrains FROM the bc2 init on the full
   aggregated dataset (v1 continued from the drifting policy).

(v1 header) DAgger-T3: teacher labels the student's OWN visited states, anchored.
...
/workspace/bc_t3_dag2_r{k}.json per round. Champion file never touched.
"""
...
def train(params, X, A, Xa, Aa, epochs, rng):
    ...
    for ep in range(epochs):
        ...
        for s in range(0, len(X), 2048):
            xa = Xa[ida[s % len(Xa):s % len(Xa) + 2048]]
            aa = Aa[ida[s % len(Xa):s % len(Xa) + 2048]]
            if len(xa) < 2048:  # wrap the anchor set
                rep = int(np.ceil(2048 / max(1, len(xa))))
                xa = np.tile(Xa, (rep, 1))[:2048]; aa = np.tile(Aa, (rep, 1))[:2048]
...
def main():
    ...
    XD, AD = [], []
    params0 = {k2: v2.copy() for k2, v2 in params.items()}
    for k in range(1, ROUNDS + 1):
        params = {k2: v2.copy() for k2, v2 in params0.items()}  # fresh from bc2 init
        ...
        json.dump(flat, open(f"/workspace/bc_t3_dag2_r{k}.json", "w"))
    ...
    flat = json.load(open(f"/workspace/bc_t3_dag2_r{ROUNDS}.json"))
    ...
    print("T3DAG2_EVAL " + json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res.items()}), flush=True)
    print("T3DAG2_DONE wall=%.0fs" % (time.time() - t0), flush=True)
