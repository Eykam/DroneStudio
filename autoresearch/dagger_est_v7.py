"""DAgger v6: v5 recipe, hover-heavy mix (70/20/10), 25 iters, from v5 best (parent 2026-09-08).
...
START = os.environ.get("DAG_START", "/workspace/bc_est_dag_v6_best.json")
...
def main():
    t0 = time.time()
    # v6 starts from a 25-dim student (v5 best), not the 19-dim champion:
    # warm_start_25 would misread the flat vector (iter0 6/0/0 bug 2026-09-08)
    flat = np.array(json.load(open(START)), dtype=np.float64)
    _n = MLP(OBS_DIM, 4, seed=0); _n.set_flat(flat)
    cur = list(_n.get_flat())
    print(f"DAGV7 start={os.path.basename(START)} recipe: lr={BC_LR} iters={BC_ITERS} anchor={GT_ANCHOR}/{EPS_PER_ITER}", flush=True)
    cells = heldout_cells()
    ...
    floors = {"goto": gt0["goto"] - 0.05, "hover_hold": gt0["hover_hold"] - 0.10,
              "land": gt0["land"] - 0.10}
    print("DAGV7 iter0 (EST v3 obs): " + json.dumps({k: round(v, 3) for k, v in res0.items()})
    ...
    for it in range(1, ITERS + 1):
        ...
        if score > best_score and floors_ok:
            ...
            best_flat = list(cur)
            json.dump(best_flat, open(OUT + "_best.json", "w"))
        print(f"DAGV7_ITER {it}: n={len(X)} wall={time.time()-tc:.0f}s EST "
    ...
    print(f"DAGV7_DONE best_mean={best_score:.3f} wall={time.time()-t0:.0f}s", flush=True)
