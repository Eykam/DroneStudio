...
(floors on tier-0 goto/hover) -> /workspace/bc_ppo_v38_best.json. Shares NO
...
# v3.8: v3.7 showed phase-A learning (+7.42 @u15) then collapse at the
# phase-B jump. Slower ramp: longer clear-slalom phase, gentler density
# steps, walls only in the final phase.
def phase_knobs(u):
    if u <= 30: return {"lat": 0.05, "dens": 0.0,  "corr": 4.0}
    if u <= 50: return {"lat": 0.10, "dens": 0.02, "corr": 4.0}
    if u <= 65: return {"lat": 0.20, "dens": 0.05, "corr": 4.0}
    return {"lat": 0.35, "dens": 0.10, "corr": 2.0}
...
def main():
    ...
    print("PPOV38 u0 (zero-pad warm start from bc_ppo_v2_best): " +
          json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res0.items()}), flush=True)
    best_mean = float(np.mean(list(res0.values())))
    best_flat = cur_flat
    json.dump(best_flat, open("/workspace/bc_ppo_v38_u0.json", "w"))
    ...
    for u in range(1, UPDATES + 1):
        ...
        args = [(a_flat, ls, c_flat, int(rng.integers(0, 2**31 - 1)), sc, knobs)
                for sc in scs]
        ...
        cur_flat = actor_to_bc_flat(policy.actor)
        json.dump(cur_flat, open(f"/workspace/bc_ppo_v38_u{u}.json", "w"))
        ...
        if mean > best_mean and floors_ok:
            best_mean = mean
            best_flat = cur_flat
            json.dump(best_flat, open("/workspace/bc_ppo_v38_best.json", "w"))
        post_series("ppo_v38_return", mean_ret, f"u{u}")
        for (sc, t), v in res.items():
            post_series(f"success_{sc}_t{t}", v, f"ppo-v3.6 u{u}")
        print(f"PPOV38 u{u}: ret={mean_ret:.2f} " +
    ...
    print(f"PPOV38_DONE best_mean={best_mean:.3f}", flush=True)
