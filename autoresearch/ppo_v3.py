...
(floors on tier-0 goto/hover) -> /workspace/bc_ppo_v3_best.json. Shares NO
...
def scene_for(seed, force_goto=False):
    """(dist, spec) for an episode seed: tier + scenario + waypoint rules."""
    tier = sample_tier(seed)
    dist = tier_dist(seed, tier)
    if tier == 3:
        force_goto = True
    spec = sample_spec(seed, force_scenario="goto" if force_goto else None)
...
def rollout_one(actor_flat, log_std, critic_flat, seed):
...
def main():
    ...
    print("PPOV3 u0 (zero-pad warm start from bc_ppo_v2_best): " +
          json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res0.items()}), flush=True)
    best_mean = float(np.mean(list(res0.values())))
    best_flat = cur_flat
    json.dump(best_flat, open("/workspace/bc_ppo_v3_u0.json", "w"))
    ...
    for u in range(1, UPDATES + 1):
        ...
        args = [(a_flat, ls, c_flat, int(rng.integers(0, 2**31 - 1)))
        ...
        cur_flat = actor_to_bc_flat(policy.actor)
        json.dump(cur_flat, open(f"/workspace/bc_ppo_v3_u{u}.json", "w"))
        ...
        if mean > best_mean and floors_ok:
            best_mean = mean
            best_flat = cur_flat
            json.dump(best_flat, open("/workspace/bc_ppo_v3_best.json", "w"))
        post_series("ppo_v3_return", mean_ret, f"u{u}")
        for (sc, t), v in res.items():
            post_series(f"success_{sc}_t{t}", v, f"ppo-v3 u{u}")
        print(f"PPOV3 u{u}: ret={mean_ret:.2f} " +
        ...
        post_status({
            ...
            "training": {"status": "running", "name": "ppo_v3", "iter": u, "iters": UPDATES,
    ...
    json.dump(best_flat, open("/workspace/bc_ppo_v3_best.json", "w"))
    post_status({
        "training": {"status": "idle", "name": "ppo_v3",
                     "note": f"finished {UPDATES} updates; best mean {best_mean:.3f}"},
        "queue": ["promotion flip of bc_ppo_v2_best: awaiting user go", "EE box #3 provisioning"],
    })
    print(f"PPOV3_DONE best_mean={best_mean:.3f}", flush=True)
