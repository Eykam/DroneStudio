...
def run(dep, meta, K, vo_every=1, dt=0.1, seed=0):
    """vo_every=N: VO correction only every Nth frame (IMU coasts between)."""
    f, cx, cy = K
    n = len(dep)
    gt_p = meta[:,1:4]
    ...
    for i in range(1, n):
        ...
        if i % vo_every == 0:
            # multi-frame VO increment: chain from last_vo_frame to i via per-frame ICP
            T_chain = np.eye(4)
            for j in range(last_vo_frame+1, i+1):
...
for sid in [1000, 1001]:
    m = meta[:,0]==sid
    for ve in [1, 5, 10, 20]:
        ate, rpe = run(dep[m], meta[m], K, vo_every=ve)
        print("scene %d VO every %2d frames: ATE %7.3f RPE %.3f" % (sid, ve, ate, rpe), flush=True)
