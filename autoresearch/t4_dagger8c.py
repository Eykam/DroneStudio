"""dag8c (2026-09-04 late): m2 obs-v4 DAgger + LAND CURRICULUM.

dag8b closed the volume hypothesis: 24 rounds with correct integrator labels
and obs v4 still left land_t0 = 0.0 in EVERY round (goto 0.938, hover 0.375
climb fine). Failure probe: the student parks at vy~=0, 0.1-0.5m above the
pad - a false equilibrium in its learned throttle map, in a state region
full-episode sampling visits for only a sliver of each rollout.

dag8c attacks the data distribution: 3 of every 10 DAgger/anchor episodes
are LAND episodes spawned in terminal descent - 0.3-3m directly above the
pad with +-1.5m horizontal offset (env_sim spawn_rel_goal). The dataset
becomes dense exactly where the equilibrium forms, with integrator-teacher
labels there. 1 of 10 remains a full-approach t2 land so the approach leg
is not forgotten. Floors, eval cells, warm-start, and champion discipline
identical to dag8b. Own champion file /workspace/t4_dag8c_best.json.
"""
...
def _mix(i):
    m = i % 10
    if m < 4:
        return "land", (0 if m < 3 else 2), m < 3   # 3 near-pad + 1 full approach
    if m < 8:
...
def main():
    ...
    for k in range(1, ROUNDS + 1):
        ...
        json.dump(flat, open(f"/workspace/t4_dag8c_r{k}.json", "w"))
        res = eval_all27(flat, f"t4-dag8c r{k}")
        ...
        P.post_status({"training": {"status": "running", "name": "t4_dagger8c", "iter": k, "iters": ROUNDS,
                       "note": f"m2 land curriculum (near-pad terminal descent); r{k} mean {mean:.3f}"}})
    print("T4DAG8C_BEST " + json.dumps(best), flush=True)
    if best["round"] > 0:
        import shutil
        shutil.copy(f"/workspace/t4_dag8b_r{best['round']}.json", "/workspace/t4_dag8c_best.json")
        # v1-plant regression read on the best (SoC pins to 1.0 on v1)
        os.environ.pop("AUTORESEARCH_MOTOR_V2", None)
        res1 = eval_all27(json.load(open("/workspace/t4_dag8c_best.json")), "t4-dag8c-best-v1plant")
        os.environ["AUTORESEARCH_MOTOR_V2"] = "1"
        m1 = float(np.mean(list(res1.values())))
        print("T4DAG8C_BEST_SAVED t4_dag8c_r%d mean_m2=%.3f v1_regression=%s mean=%.3f" % (
            best["round"], best["mean"],
            json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res1.items()}), m1), flush=True)
    P.post_status({"training": {"status": "idle", "name": "t4_dagger8c",
                   "note": f"m2 obs-v4 land-curriculum DAgger done; best round {best['round']} mean {best['mean']:.3f}"}})
    print("T4DAG8C_DONE wall=%.0fs" % (time.time() - t0), flush=True)
