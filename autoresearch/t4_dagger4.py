"""T4 stage 4: land-precision recovery DAgger from t4_best (= landfix2 dag2_r4).
...
best passing round (max mean) becomes t4_best. Outputs t4_dag4_r{k}.json.

Init: /workspace/t4_best.json. Saves /workspace/t4_dag4_r{k}.json.
"""
...
def dagger_rollout(params, actor, episodes, seed0):
    ...
    for i in range(episodes):
        ...
        m = i % 10
        if m < 6:
            sc, tier = "land", (0 if m < 4 else m - 4)  # 4x T0, 1x T1, 1x T2
        elif m < 8:
...
def main():
    ...
    for k in range(1, ROUNDS + 1):
        ...
        X, A, succ = dagger_rollout(params, actor, episodes=30, seed0=250000 + 1000 * k)
        XD += X; AD += A
        ...
        json.dump(flat, open(f"/workspace/t4_dag4_r{k}.json", "w"))
        res = P.eval_all(flat, f"t4-dag4 r{k}")
    ...
    print("T4DAG4_BEST " + json.dumps(best), flush=True)
    if best["round"] > 0:
        import shutil
        shutil.copy(f"/workspace/t4_dag4_r{best['round']}.json", "/workspace/t4_best.json")
        print(f"T4BEST_UPDATED t4_dag4_r{best['round']} mean={best['mean']:.3f}", flush=True)
    P.post_status({"training": {"status": "idle", "name": "t4_dagger4",
                   "note": f"T4 land-precision DAgger done; best round {best['round']} mean {best['mean']:.3f}"}})
    print("T4DAG4_DONE wall=%.0fs" % (time.time() - t0), flush=True)
