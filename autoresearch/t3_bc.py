"""BC the T3 teacher into the waypoint pathway only.
...
Output: /workspace/bc_t3_wp.json - pack_actor layout (trunk flat + wp tail),
...
from scenario_sampler import sample_spec, heldout_cells_tiered
...
def main():
    ...
    t0 = time.time()
    X, A, kept = collect()
    print(f"BC data: {len(X)} samples from {kept} successful demos", flush=True)

    champ = P.warm_start_params()          # trunk dict (w1 zero-padded)
    ...
    json.dump(flat, open("/workspace/bc_t3_wp.json", "w"))
    ...
    res = {("goto", 3): run_cell("goto", 3), ("goto", 0): run_cell("goto", 0),
           ("hover_hold", 0): run_cell("hover_hold", 0), ("land", 0): run_cell("land", 0)}
    print("T3BC_EVAL " + json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res.items()}), flush=True)
    ...
    for ph in ("A", "B"):
        ...
        for i in range(16):
            ...
            env.close()
        print(f"T3BC_CLONE phase {ph}: succ {np.mean(oks):.3f} wp_frac {np.mean(wps):.3f}", flush=True)
    print("T3BC_DONE wall=%.0fs" % (time.time() - t0), flush=True)
