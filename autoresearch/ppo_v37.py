...
(floors on tier-0 goto/hover) -> /workspace/bc_ppo_v37_best.json. Shares NO
...
# T3 full-difficulty curriculum: A = clear gentle slalom (signal-dense,
# no crashes), B = moderate offset + light clutter, C = full T3 (walls).
def phase_knobs(u):
    if u <= 20: return {"lat": 0.05, "dens": 0.0,  "corr": 4.0}
    if u <= 40: return {"lat": 0.15, "dens": 0.05, "corr": 4.0}
...
def main():
    ...
    print("PPOV37 u0 (zero-pad warm start from bc_ppo_v2_best): " +
          json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res0.items()}), flush=True)
    best_mean = float(np.mean(list(res0.values())))
    best_flat = cur_flat
    json.dump(best_flat, open("/workspace/bc_ppo_v37_u0.json", "w"))
    ...
    for u in range(1, UPDATES + 1):
        ...
        cur_flat = actor_to_bc_flat(policy.actor)
        json.dump(cur_flat, open(f"/workspace/bc_ppo_v37_u{u}.json", "w"))
        ...
        if mean > best_mean and floors_ok:
            best_mean = mean
            best_flat = cur_flat
            json.dump(best_flat, open("/workspace/bc_ppo_v37_best.json", "w"))
        post_series("ppo_v37_return", mean_ret, f"u{u}")
        for (sc, t), v in res.items():
            post_series(f"success_{sc}_t{t}", v, f"ppo-v3.6 u{u}")
        print(f"PPOV37 u{u}: ret={mean_ret:.2f} " +
    ...
    print(f"PPOV37_DONE best_mean={best_mean:.3f}", flush=True)
