"""T4 stage 3b: precision-focused DAgger top-up from t4_dag2_r6.

Full-net PPO (t4_ppo/2/3/4: LR 3e-4 and 3e-5, curriculum, PBRS, T0-matched
tiers) keeps losing hover 0.5->0 within ~15 updates - exploration noise
(std 0.05) makes a borderline hold fail, hover episodes score below
average, and the gradient walks away from holding. DAgger is the method
that moved hover at all (0->0.5): student rolls ONLY hover/land across
T0-T2 (the weak cells; land demos were thin at t2 6/48 but the teacher
lands 47/48 T0, 40/48 T1), teacher labels, retrain from the t4_dag2_r6 init
each round, 25% original-demo anchor guards goto.

Init: /workspace/t4_dag2_r6.json. Saves /workspace/t4_dag2_r{k}.json.

(t4_dagger header) T4 stage 3: DAgger on the fresh HID-64 unified policy - ALL tiers.
...
Init: /workspace/t4_bc.json. Saves /workspace/t4_dag2_r{k}.json per round,
...
def dagger_rollout(params, actor, episodes, seed0):
    ...
    for i in range(episodes):
        seed = seed0 + i
        tier = i % 3                    # T0-T2 only - T3 goto is healthy
        sc = ("hover_hold", "land", "hover_hold", "land", "goto")[i % 5]
...
def main():
    ...
    for k in range(1, ROUNDS + 1):
        ...
        X, A, succ = dagger_rollout(params, actor, episodes=30, seed0=130000 + 1000 * k)
        XD += X; AD += A
        ...
        json.dump(flat, open(f"/workspace/t4_dag2_r{k}.json", "w"))
        res = P.eval_all(flat, f"t4-dag2 r{k}")
        print(f"round {k} EVAL " + json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res.items()}), flush=True)
    P.post_status({"training": {"status": "idle", "name": "t4_dagger2",
                   "note": "T4 stage 3b (precision DAgger x8) done"}})
    print("T4DAG2_DONE wall=%.0fs" % (time.time() - t0), flush=True)
