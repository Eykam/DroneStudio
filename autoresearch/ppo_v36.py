...
(floors on tier-0 goto/hover) -> /workspace/bc_ppo_v36_best.json. Shares NO
...
# T3 curriculum: near-beeline waypoints first (the warm-start policy already
# passes them -> immediate waypoint-pass gradient), then open up the slalom
def wp_lat_for(u):
    if u <= 15: return 0.05
    if u <= 35: return 0.15
    return 0.35
...
def scene_for(seed, scenario, t3_boost, rng):
    ...
    # v3.6: T3-only + curriculum on lateral offset (module-level phase var)
    tier = 3
    scenario = "goto"
    dist = tier_dist(seed, 3)
...
def rollout_one(actor_flat, log_std, critic_flat, seed, scenario, wp_lat):
    ...
    dist, spec, tier = scene_for(seed, scenario, 0, rng)
    dist.waypoint_lat = wp_lat  # curriculum phase from main()
...
def main():
    ...
    print("PPOV36 u0 (zero-pad warm start from bc_ppo_v2_best): " +
          json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res0.items()}), flush=True)
    best_mean = float(np.mean(list(res0.values())))
    best_flat = cur_flat
    json.dump(best_flat, open("/workspace/bc_ppo_v36_u0.json", "w"))
    ...
    for u in range(1, UPDATES + 1):
        ...
        args = [(a_flat, ls, c_flat, int(rng.integers(0, 2**31 - 1)), sc, wp_lat)
                for sc in scs]
        ...
        cur_flat = actor_to_bc_flat(policy.actor)
        json.dump(cur_flat, open(f"/workspace/bc_ppo_v36_u{u}.json", "w"))
        ...
        if mean > best_mean and floors_ok:
            best_mean = mean
            best_flat = cur_flat
            json.dump(best_flat, open("/workspace/bc_ppo_v36_best.json", "w"))
        post_series("ppo_v36_return", mean_ret, f"u{u}")
        for (sc, t), v in res.items():
            post_series(f"success_{sc}_t{t}", v, f"ppo-v3.6 u{u}")
        print(f"PPOV36 u{u}: ret={mean_ret:.2f} " +
    ...
    print(f"PPOV36_DONE best_mean={best_mean:.3f}", flush=True)
