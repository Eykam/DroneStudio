"""
dag12b (2026-09-05 PM): LAND-FOCUSED DAgger - experiment (b) of the user
...
t4_best lineage untouched, own champion file /workspace/t4_dag12b_best.json).
...
def _mix(i):
    # dag12b: land-only - 7 near-pad t0, 1 near-pad t1, 2 full-approach t2
    m = i % 10
    if m < 7:
        return "land", 0, True
    if m < 8:
...
def _term_w(obs, ext, spec):
    """dag12b terminal-phase weight: 4.0 inside the teacher engagement gate."""
...
def unpack27(flat):
    """27-dim packed flat (pack27) -> params dict; for continuing a run."""
    n_wp = WP_IN * WP_HID + WP_HID * ACT_DIM
    wp2 = np.array(flat[-WP_HID * ACT_DIM:], dtype=np.float64).reshape(WP_HID, ACT_DIM)
    wp1 = np.array(flat[-WP_HID * ACT_DIM - WP_IN * WP_HID:-WP_HID * ACT_DIM], dtype=np.float64).reshape(WP_IN, WP_HID)
    trunk = np.array(flat[:-n_wp], dtype=np.float64)
    from policy import MLP as BCMlp
    bc = BCMlp(OBS_DIM, ACT_DIM, hidden=HID, seed=0)
    bc.set_flat(trunk)
...
def main():
    ...
    t0 = time.time()
    # dag12b: continue from dag12b r24 (land_t0 0.562 and still climbing) -
    ...
    for k in range(1, ROUNDS + 1):
        ...
        _ic(1)
        X, A, W, succ = dagger_rollout(params, actor, episodes=30, seed0=724000 + 1000 * k)
        _ic(0)
        XD += X; AD += A; WD += W
        ...
        json.dump(flat, open(f"/workspace/t4_dag12b_r{k}.json", "w"))
        res = eval_all27(flat, f"t4-dag12b r{k}")
        _ic(1)
        res_ic = eval_all27(flat, f"t4-dag12b-ic r{k}")
        _ic(0)
        ...
        P.post_status({"training": {"status": "running", "name": "t4_dagger11", "iter": k, "iters": ROUNDS, "note": "YARDSTICK CHANGE: skim penalty; dag12b+ land numbers not comparable to dag7-9",
                       "note": f"m2 land curriculum + IC v1; r{k} mean {mean:.3f}"}})
    print("T4DAG12B_BEST " + json.dumps(best), flush=True)
    if best["round"] > 0:
        import shutil
        shutil.copy(f"/workspace/t4_dag12b_r{best['round']}.json", "/workspace/t4_dag12b_best.json")
        # v1-plant regression read on the best (SoC pins to 1.0 on v1)
        os.environ.pop("AUTORESEARCH_MOTOR_V2", None)
        _ic(0)
        res1 = eval_all27(json.load(open("/workspace/t4_dag12b_best.json")), "t4-dag12b-best-v1plant")
        os.environ["AUTORESEARCH_MOTOR_V2"] = "1"
        m1 = float(np.mean(list(res1.values())))
        print("T4DAG12B_BEST_SAVED t4_dag12b_r%d mean_m2=%.3f v1_regression=%s mean=%.3f" % (
    ...
    print("T4DAG12B_DONE wall=%.0fs" % (time.time() - t0), flush=True)
