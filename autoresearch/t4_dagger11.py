...
3. DUAL EVAL each round: standard rest-start cells (floors + champion
   gating, comparable to dag7/8 history, label t4-dag11) and the same
   cells under IC v1 spawns as a secondary robustness read
   (label t4-dag11-ic). Floors are judged on the standard eval only.

Own champion file /workspace/t4_dag11_best.json.
"""
...
def main():
    ...
    for k in range(1, ROUNDS + 1):
        ...
        json.dump(flat, open(f"/workspace/t4_dag11_r{k}.json", "w"))
        res = eval_all27(flat, f"t4-dag11 r{k}")
        _ic(1)
        res_ic = eval_all27(flat, f"t4-dag11-ic r{k}")
        _ic(0)
        ...
        P.post_status({"training": {"status": "running", "name": "t4_dagger11", "iter": k, "iters": ROUNDS, "note": "YARDSTICK CHANGE: skim penalty; dag11+ land numbers not comparable to dag7-9",
                       "note": f"m2 land curriculum + IC v1; r{k} mean {mean:.3f}"}})
    print("T4DAG11_BEST " + json.dumps(best), flush=True)
    if best["round"] > 0:
        import shutil
        shutil.copy(f"/workspace/t4_dag11_r{best['round']}.json", "/workspace/t4_dag11_best.json")
        # v1-plant regression read on the best (SoC pins to 1.0 on v1)
        os.environ.pop("AUTORESEARCH_MOTOR_V2", None)
        _ic(0)
        res1 = eval_all27(json.load(open("/workspace/t4_dag11_best.json")), "t4-dag11-best-v1plant")
        os.environ["AUTORESEARCH_MOTOR_V2"] = "1"
        m1 = float(np.mean(list(res1.values())))
        print("T4DAG11_BEST_SAVED t4_dag11_r%d mean_m2=%.3f v1_regression=%s mean=%.3f" % (
    ...
    print("T4DAG11_DONE wall=%.0fs" % (time.time() - t0), flush=True)
