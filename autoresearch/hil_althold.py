...
AKP = float(sys.argv[1]) if len(sys.argv) > 1 else 15.0
AKI = float(sys.argv[2]) if len(sys.argv) > 2 else 4.0
AKD = float(sys.argv[3]) if len(sys.argv) > 3 else 8.0
TARGET_ALT = float(sys.argv[4]) if len(sys.argv) > 4 else 1.5
SPAWN_ALT = float(sys.argv[5]) if len(sys.argv) > 5 else 1.5
...
call({'cmd': 'reset', 'seed': 42, 'scene': {'spawn': [0.0, SPAWN_ALT, 0.0], 'goal': [0.0, 0.0, 0.0], 'obstacles': [], 'extent': 10, 'max_steps': 2000}})
...
while time.time() - t0 < DUR:
    ...
    n += 1
    out.append((time.time() - t0, pos[1], vel[1]))
    time.sleep(max(0, 0.01 - (time.time() - t0 - n * 0.01)))
alts = [a for _, a, _ in out]
late = [a for t, a, _ in out if t > 2.0]
rmse = math.sqrt(sum((a - TARGET_ALT)**2 for a in late) / max(1, len(late)))
print(f'ALTHOLD kp={AKP} ki={AKI} kd={AKD} target={TARGET_ALT}: end={alts[-1]:.3f} late_rmse={rmse:.3f} late_min={min(late):.3f} late_max={max(late):.3f}')
print('ALT(0.4s):', [round(a,2) for a in alts[::40]])
