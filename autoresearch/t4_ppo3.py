"""PPO T4.3: potential-based reward shaping (PBRS) for precision scenarios.
...
def main():
    ...
    res0 = P.eval_all(cur_flat, "t4-ppo3 u0")
    print("T4PPO3 u0 (t4_dag_r6 init): " +
          json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res0.items()}), flush=True)
    best_mean = -1e9
    best_flat = cur_flat
    json.dump(best_flat, open("/workspace/t4_ppo3_u0.json", "w"))
    ...
    for u in range(1, UPDATES + 1):
        ...
        cur_flat = P.pack_actor(policy.actor, WP1, WP2)
        json.dump(cur_flat, open(f"/workspace/t4_ppo3_u{u}.json", "w"))
        res = P.eval_all(cur_flat, f"t4-ppo3 u{u}")
        mean = float(np.mean(list(res.values())))
        floors_ok = res[("goto", 0)] >= 0.9 and res[("hover_hold", 0)] >= 0.6
        if floors_ok and mean > best_mean:
            best_mean = mean
            best_flat = cur_flat
            json.dump(best_flat, open("/workspace/t4_ppo3_best.json", "w"))
        P.post_series("t4_ppo3_return", mean_ret, f"u{u}")
        print(f"T4PPO3 u{u}: ret={mean_ret:.2f} " +
        ...
        P.post_status({
            "training": {"status": "running", "name": "t4_ppo3", "iter": u, "iters": UPDATES,
    ...
    json.dump(best_flat, open("/workspace/t4_ppo3_best.json", "w"))
    P.post_status({"training": {"status": "idle", "name": "t4_ppo3",
                                "note": f"T4 HID-64 PPO finished {UPDATES} updates; best mean {best_mean:.3f}"}})
    print(f"T4PPO3_DONE best_mean={best_mean:.3f}", flush=True)
