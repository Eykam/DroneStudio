"""PPO T4: full-net PPO on the fresh HID-64 unified bootstrap.

Yaw-facing shaping variant (user directive 2026-09-04: "add a slight
reward for facing object"): t4_ppo + a small cos(bearing) reward term
(lambda via T4_YAW_LAMBDA, default 0.01; max +lambda/step vs 50 outcome /
10 wp-pass - outcome-dominant balance preserved), init from t4_best.json
(t4_dag3_r1), LR 3e-5. Floors still guard precision (goto_t0 >= 0.9 AND
hover_hold_t0 >= 0.6); aborts after 8 consecutive floor failures.
Original header: init from the t4 DAgger
...
ALIVE_BONUS = 0.02
MIX = ("land", "land", "land", "hover_hold", "goto", "goto")  # land-heavy forgetting guard
...
def rollout_one(actor_flat, log_std, critic_flat, seed, scenario, knobs):
    ...
    while not done:
        ...
        # yaw-facing shaping (small): +lambda/2*(1+cos(bearing)) when the
        # target is far enough for the bearing to be meaningful
...
def main():
    ...
    init = json.load(open("/workspace/t4_best.json"))
    ...
    res0 = P.eval_all(cur_flat, "t4-ppo5 u0")
    print("T4PPO5 u0 (t4_best init): " +
          json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res0.items()}), flush=True)
    best_mean = -1e9
    best_flat = cur_flat
    json.dump(best_flat, open("/workspace/t4_ppo5_u0.json", "w"))
    floor_fails = 0
    ...
    for u in range(1, UPDATES + 1):
        ...
        cur_flat = P.pack_actor(policy.actor, WP1, WP2)
        json.dump(cur_flat, open(f"/workspace/t4_ppo5_u{u}.json", "w"))
        res = P.eval_all(cur_flat, f"t4-ppo5 u{u}")
        ...
        if floors_ok and mean > best_mean:
            best_mean = mean
            best_flat = cur_flat
            json.dump(best_flat, open("/workspace/t4_ppo5_best.json", "w"))
        P.post_series("t4_ppo5_return", mean_ret, f"u{u}")
        P.post_series("t4_ppo5_face", mean_face, f"u{u}")
        if floor_fails > 8:
            print(f"T4PPO5 ABORT u{u}: floors failed 8 consecutive updates "
                  f"(precision collapse guard)", flush=True)
            break
        print(f"T4PPO5 u{u}: ret={mean_ret:.2f} face={mean_face:.3f} " +
        ...
        P.post_status({
            "training": {"status": "running", "name": "t4_ppo5", "iter": u, "iters": UPDATES,
    ...
    json.dump(best_flat, open("/workspace/t4_ppo_best.json", "w"))
    P.post_status({"training": {"status": "idle", "name": "t4_ppo",
                                "note": f"T4 HID-64 PPO finished {UPDATES} updates; best mean {best_mean:.3f}"}})
    print(f"T4PPO_DONE best_mean={best_mean:.3f}", flush=True)
