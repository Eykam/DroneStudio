...
def run(label, rp, yaw_g, steps=700, seed=80000):
    ...
    obs = env.reset()
    env._call({"cmd": "set_gains", "roll": rp, "pitch": rp, "yaw": yaw_g})
    ...
    for step in range(steps):
        ...
        vel = obs[3:6] * 10.0
        # althold only (no outer attitude loop) + sustained yaw: isolates the rate PID
    ...
    print(f"{label} rp={rp} yaw={yaw_g}: died_at={died} succ={env.succeeded} maxring={maxring:.2f}", flush=True)
    env.close()
run("stock", [0.1, 0.005, 0.001], [0.05, 0.003, 0.0005])
run("med", [0.25, 0.01, 0.003], [0.05, 0.003, 0.0005])
run("stiff", [0.5, 0.02, 0.008], [0.05, 0.003, 0.0005])
run("vstiff", [1.0, 0.05, 0.02], [0.05, 0.003, 0.0005])
run("stiff+yawstiff", [0.5, 0.02, 0.008], [0.15, 0.01, 0.002])
