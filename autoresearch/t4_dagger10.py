...
3. DUAL EVAL each round: standard rest-start cells (floors + champion
   gating, comparable to dag7/8 history, label t4-dag10) and the same
   cells under IC v1 spawns as a secondary robustness read
   (label t4-dag10-ic). Floors are judged on the standard eval only.

Own champion file /workspace/t4_dag10_best.json.
"""
...
def main():
    ...
    for k in range(1, ROUNDS + 1):
        ...
        json.dump(flat, open(f"/workspace/t4_dag10_r{k}.json", "w"))
        res = eval_all27(flat, f"t4-dag10 r{k}")
        _ic(1)
        res_ic = eval_all27(flat, f"t4-dag10-ic r{k}")
        _ic(0)
    ...
    if best["round"] > 0:
        import shutil
        shutil.copy(f"/workspace/t4_dag10_r{best['round']}.json", "/workspace/t4_dag10_best.json")
        # v1-plant regression read on the best (SoC pins to 1.0 on v1)
        os.environ.pop("AUTORESEARCH_MOTOR_V2", None)
        _ic(0)
        res1 = eval_all27(json.load(open("/workspace/t4_dag10_best.json")), "t4-dag10-best-v1plant")
        os.environ["AUTORESEARCH_MOTOR_V2"] = "1"
        m1 = float(np.mean(list(res1.values())))
        print("T4DAG10_BEST_SAVED t4_dag10_r%d mean_m2=%.3f v1_regression=%s mean=%.3f" % (
            best["round"], best["mean"],
            json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res1.items()}), m1), flush=True)
    P.post_status({"training": {"status": "idle", "name": "t4_dagger10",
