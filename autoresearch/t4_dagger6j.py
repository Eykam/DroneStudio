"""TRACK 2 (user-approved 2026-09-04): dynamics-jitter robustness DAgger.

Same recipe as dag5 (40/40/20 land/hover/goto, land T0-biased, 100 epochs,
floors goto_t0>=0.9 AND hover_t0>=0.6 AND land_t0>=0.7) but rollout
airframes are jittered per episode seed (dynamics_jitter.py, T10): mass
+/-5%, inertia +/-8%, per-motor thrust +/-4%, lag +/-25%, drag +/-15%,
CoM +/-3mm. Eval cells stay on the BASE airframe (jitter off during
...
/workspace/t4_dag6j_best.json - does NOT touch t4_best.json (track 1).
Outputs t4_dag6j_r{k}.json.
"""
...
def main():
    ...
    for k in range(1, ROUNDS + 1):
        ...
        X, A, succ = dagger_rollout(params, actor, episodes=30, seed0=400000 + 1000 * k)
        XD += X; AD += A
        ...
        json.dump(flat, open(f"/workspace/t4_dag6j_r{k}.json", "w"))
        res = P.eval_all(flat, f"t4-dag6j r{k}")
    ...
    print("T4DAG6J_BEST " + json.dumps(best), flush=True)
    if best["round"] > 0:
        import shutil
        shutil.copy(f"/workspace/t4_dag6j_r{best['round']}.json", "/workspace/t4_dag6j_best.json")
        print(f"T4DAG6J_BEST_SAVED t4_dag6j_r{best['round']} mean={best['mean']:.3f}", flush=True)
    P.post_status({"training": {"status": "idle", "name": "t4_dagger6j",
                   "note": f"T4 jitter-robustness DAgger done; best round {best['round']} mean {best['mean']:.3f}"}})
    print("T4DAG6J_DONE wall=%.0fs" % (time.time() - t0), flush=True)
