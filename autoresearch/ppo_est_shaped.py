"""PPO under ESTIMATED obs with DENSE per-step shaping (hover/land rescue).

Instead of outcome-only ALIVE_BONUS, the per-step reward is the binarys
true shaped signal (progress, drift penalties, binary outcome terms) while
the policy sees estimated obs. Direct recipe from bc_ppo_est_best.
...
OUT = "/workspace/bc_ppo_est_shaped"
...
def rollout_one(actor_flat, log_std, critic_flat, seed, scenario):
    ...
    while not done:
        ...
        # dense shaping: the binary's true per-step reward (progress, drift
        # penalties, +10/-5 outcomes) - hover/land get gradient even at 0%
        # success; outcome-only ALIVE_BONUS starved them (measured).
...
def main():
    ...
    for u in range(1, UPDATES + 1):
        ...
        if mean > best_mean and floors_ok:
            ...
            best_flat = cur_flat
            json.dump(best_flat, open(OUT + "_best.json", "w"))
        post_series("ppo_est_shaped_return", mean_ret, f"u{u}")
        ...
        print(f"PPOEST u{u}: ret={mean_ret:.2f} " +
              ...
              f" gt_goto={gtg:.3f} best_est={best_mean:.3f} wall={time.time()-t0:.0f}s", flush=True)
        post_status({
            "live_policy": {"name": "bc_ppo_v2_best.json",
                            "detail": "obs v2 (19-dim) GT-trained champion",
                            "note": "flying on /watch"},
            "candidate": {"name": "bc_ppo_est_shaped_best.json",
            ...
            "training": {"status": "running", "name": "ppo_est_shaped", "iter": u, "iters": UPDATES,
    ...
    json.dump(best_flat, open(OUT + "_best.json", "w"))
    post_status({
        "training": {"status": "complete", "name": "ppo_est_shaped", "iter": UPDATES, "iters": UPDATES,
