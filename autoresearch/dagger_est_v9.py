...
so composite best stayed the warm start. v8 changes:
  ...
  - per-phase best-keeping: bc_est_dag_v9_best_{goto,hover_hold,land}.json
...
START = os.environ.get("DAG_START", "/workspace/bc_est_dag_v8_best_hover_hold.json")
ITERS = int(os.environ.get("DAG_ITERS", "25"))
EPS_PER_ITER = int(os.environ.get("DAG_EPS", "32"))
GT_ANCHOR = 8
SCEN_MIX = ("land",) * 12 + ("hover_hold",) * 5 + ("goto",) * 3  # hover-heavy, parent 2026-09-08
OUT = "/workspace/bc_est_dag_v9"
OBS_DIM = 25
BC_ITERS = 300
BC_LR = 5e-4
...
def main():
    ...
    print(f"DAGV9 start={os.path.basename(START)} recipe: lr={BC_LR} iters={BC_ITERS} anchor={GT_ANCHOR}/{EPS_PER_ITER} per-phase-best", flush=True)
    cells = heldout_cells()
    ...
    floors = {"goto": gt0["goto"] - 0.05, "hover_hold": gt0["hover_hold"] - 0.10,
              "land": gt0["land"] - 0.10}
    print("DAGV9 iter0 (EST v3 obs): " + json.dumps({k: round(v, 3) for k, v in res0.items()})
    ...
    for it in range(1, ITERS + 1):
        ...
        for sc in ph_best:
            if res[sc] > ph_best[sc][0]:
                ...
                json.dump(list(cur), open(f"{OUT}_best_{sc}.json", "w"))
        print(f"DAGV9_ITER {it}: n={len(X)} wall={time.time()-tc:.0f}s EST "
        ...
        for sc, v in res.items():
            post_series(f"est_success_{sc}", v, label=f"dag-v9 i{it}")
        post_status({
            "candidate": {"name": "bc_est_dag_v9_best.json",
                          ...
                          "note": f"dag-v9 i{it} EST | GT g{gt['goto']:.2f} h{gt['hover_hold']:.2f} l{gt['land']:.2f}"},
    ...
    for name, path in kept.items():
        ...
        print("DAGV9_FINAL " + name + ": " + json.dumps({k: round(v, 3) for k, v in matrix[name].items()}), flush=True)
    ...
    print(f"DAGV9_DONE best_mean={best_score:.3f} wall={time.time()-t0:.0f}s", flush=True)
