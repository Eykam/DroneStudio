...
MODE = sys.argv[1] if len(sys.argv) > 1 else 'detour'  # naive | detour
DT = 0.080
# obstacles: sim frame [x, y(up), z, r] - block the direct square legs
OBS = [[0.5, 1.5, 0.0, 0.25], [1.0, 1.5, 0.5, 0.25]]
if MODE == 'naive':
    WPS = [(1.0, 0.0), (1.0, 1.0), (0.0, 0.0)]  # FC frame (x fwd, y right); y->sim z
else:
    WPS = [(0.0, -0.7), (1.0, -0.7), (1.0, 0.0), (1.7, 0.5), (1.0, 1.0), (0.3, 1.0), (0.0, 0.0)]
ARRIVE = 0.25; DWELL = 0.5; LEG_TIMEOUT = 10.0
...
call({'cmd': 'reset', 'seed': 42, 'scene': {'spawn': [0.0, 1.5, 0.0], 'goal': [9.0, 1.5, 9.0], 'obstacles': OBS, 'extent': 10, 'max_steps': 4000}})
...
t0 = time.time(); n = 0
min_clear = 1e9; leg_times = []; collided_reply = False
MAXT = 60.0
while time.time() - t0 < MAXT and wi < len(WPS):
    ...
    rep = call({'cmd': 'hil_step', 'ticks': 5})
    if isinstance(rep, dict) and rep.get('info', {}).get('collided'): collided_reply = True
    n += 1
    for ox, oy, oz, orad in OBS:
        d = math.sqrt((pos[0]-ox)**2 + (pos[1]-oy)**2 + (pos[2]-oz)**2) - orad
    ...
    if err < ARRIVE:
        if dwell_start is None: dwell_start = time.time()
            ...
            leg_times.append(round(time.time() - leg_start, 1))
            wi += 1
            if wi < len(WPS):
                send(f'SetPosition {WPS[wi][0]} {WPS[wi][1]}')
                leg_start = time.time(); dwell_start = None
...
done_flag = stj.get('done', None)
print(f'NAVOBS mode={MODE}: legs_done={wi}/{len(WPS)} leg_times={leg_times} min_clearance={min_clear:.3f}m collided_sim={collided_reply} final_pos=({pos[0]:.2f},{pos[2]:.2f})')
