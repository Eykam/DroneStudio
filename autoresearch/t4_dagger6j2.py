"""TRACK 2 run 2 (2026-09-04): anchored dynamics-jitter DAgger (dag6j2).

dag6j run 1 (full-strength jitter on 100% of rollout episodes) failed all
floors: hover held (0.875-0.938) but goto/land base-airframe precision
collapsed (final mean 0.597 vs dag5 0.799). Run 2 applies the standard
anchor fix: HALF-STRENGTH jitter (AUTORESEARCH_DYN_JITTER_SCALE=0.5) on
only 50% of rollout episodes (AUTORESEARCH_DYN_JITTER_MIX=0.5,
deterministic per-seed flip), the other 50% fly the base airframe.

Same recipe as dag5/dag6j otherwise (40/40/20 land/hover/goto, land
T0-biased, 100 epochs, floors goto_t0>=0.9 AND hover_t0>=0.6 AND
land_t0>=0.7). Eval cells stay on the BASE airframe (jitter off during
eval_all) so dashboard series stay comparable with the whole campaign.

Init: /workspace/t4_best.json (= dag5_r2, mean 0.799). Own champion file
/workspace/t4_dag6j2_best.json - does NOT touch t4_best.json (track 1).
Outputs t4_dag6j2_r{k}.json.
"""
...
def dagger_rollout(params, actor, episodes, seed0):
    os.environ["AUTORESEARCH_DYN_JITTER"] = "1"  # rollout airframes only
    os.environ["AUTORESEARCH_DYN_JITTER_SCALE"] = "0.5"
    os.environ["AUTORESEARCH_DYN_JITTER_MIX"] = "0.5"
    X, A, succ = [], [], []
    ...
    for i in range(episodes):
        ...
        env.close()
    os.environ.pop("AUTORESEARCH_DYN_JITTER", None)
    os.environ.pop("AUTORESEARCH_DYN_JITTER_SCALE", None)
    os.environ.pop("AUTORESEARCH_DYN_JITTER_MIX", None)
...
def main():
    ...
    for k in range(1, ROUNDS + 1):
        ...
        X, A, succ = dagger_rollout(params, actor, episodes=30, seed0=500000 + 1000 * k)
        XD += X; AD += A
        ...
        json.dump(flat, open(f"/workspace/t4_dag6j2_r{k}.json", "w"))
        res = P.eval_all(flat, f"t4-dag6j2 r{k}")
    ...
    print("T4DAG6J2_BEST " + json.dumps(best), flush=True)
    if best["round"] > 0:
        import shutil
        shutil.copy(f"/workspace/t4_dag6j_r{best['round']}.json", "/workspace/t4_dag6j2_best.json")
        print(f"T4DAG6J2_BEST_SAVED t4_dag6j2_r{best['round']} mean={best['mean']:.3f}", flush=True)
    P.post_status({"training": {"status": "idle", "name": "t4_dagger6j2",
                   "note": f"T4 anchored-jitter DAgger done; best round {best['round']} mean {best['mean']:.3f}"}})
    print("T4DAG6J2_DONE wall=%.0fs" % (time.time() - t0), flush=True)
