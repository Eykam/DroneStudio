"""DAgger: student rolls out, teacher (scripted pilot + avoidance) labels the
states the student actually visits, aggregate, retrain. Fixes BC's core
failure: the student drifts off the demo distribution and meets states the
teacher never showed it.
"""
import sys, json
sys.path.insert(0, "/workspace")
sys.path.insert(0, "/workspace/DroneStudio/autoresearch")
import numpy as np
from scene_schema import SceneDistribution
from env_sim import make_sim_factory
from policy import MLP
from diverse_bc import pilot_act2, bc_train, eval_net

MANIFEST = "/workspace/DroneStudio/autoresearch/fixtures/chassis_v1.manifest.json"
rng = np.random.default_rng(11)

def diverse_dist():
    gd = float(rng.choice([2, 5, 10, 15]))
    dens = float(rng.choice([0.0, 0.05, 0.1, 0.2]))
    return SceneDistribution(obstacle_density=dens, corridor_width=4.0,
        scene_extent=max(10.0, gd * 2), goal_distance=gd, light_direction_entropy=0.3,
        texture_variety=0.0, dynamics_noise=0.0)

def student_rollouts(net, n_eps=16):
    X, Y = [], []
    for _ in range(n_eps):
        env = make_sim_factory(diverse_dist(), max_steps=400, dynamics=MANIFEST)(int(rng.integers(1, 1 << 30)))
        obs = env.reset()
        for _ in range(400):
            X.append(obs.copy())
            Y.append(pilot_act2(obs, 1.5).copy())   # teacher label on student-visited state
            obs, r, done = env.step(net.act(obs))
            if done:
                break
        env.close()
    return X, Y

# seed dataset: original narrow demos if present, else teacher-only demos
try:
    d = np.load("/workspace/demo_diverse.npz")
    X = list(d["X"]); Y = list(d["Y"])
except Exception:
    X, Y = [], []

net = MLP(15, 4, seed=0)
net.set_flat(np.array(json.load(open("/workspace/bc_flat.json")), dtype=np.float64))

best = (0.333, "/workspace/bc_flat.json")  # narrow-BC reference mean over 2/5/10 d0
for it in range(1, 6):
    xs, ys = student_rollouts(net, n_eps=16)
    X += xs; Y += ys
    if len(X) > 20000:
        X, Y = X[-20000:], Y[-20000:]
    net = bc_train(np.array(X), np.clip(np.array(Y), -0.95, 0.95), iters=1500)
    s2, _ = eval_net(net, 2.0, 0.0)
    s5, _ = eval_net(net, 5.0, 0.0)
    s10, _ = eval_net(net, 10.0, 0.0)
    s5o, _ = eval_net(net, 5.0, 0.1)
    mean3 = (s2 + s5 + s10) / 3
    print(f"DAGGER_ITER {it}: n={len(X)} succ 2m={s2:.3f} 5m={s5:.3f} 10m={s10:.3f} 5m+d0.1={s5o:.3f} mean3={mean3:.3f}", flush=True)
    if mean3 > best[0]:
        json.dump(net.get_flat().tolist(), open("/workspace/bc_flat_dagger.json", "w"))
        best = (mean3, "/workspace/bc_flat_dagger.json")
        print(f"  new best -> {best[1]}", flush=True)
print("DAGGER_DONE", json.dumps({"best_mean3": best[0], "path": best[1]}), flush=True)
