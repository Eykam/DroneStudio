...
LR, CLIP, GAMMA, LAM = 3e-4, 0.2, 0.999, 0.95
...
def main():
    ...
    res0 = P.eval_all(cur_flat, "t4-ppo u0")
    print("T4PPO u0 (t4_dag_r6 init): " +
          json.dumps({f"{sc}_t{t}": round(v, 3) for (sc, t), v in res0.items()}), flush=True)
    best_mean = -1e9
    best_flat = cur_flat
    json.dump(best_flat, open("/workspace/t4_ppo_u0.json", "w"))
    ...
    for u in range(1, UPDATES + 1):
        ...
        cur_flat = P.pack_actor(policy.actor, WP1, WP2)
        json.dump(cur_flat, open(f"/workspace/t4_ppo_u{u}.json", "w"))
        res = P.eval_all(cur_flat, f"t4-ppo u{u}")
        ...
        P.post_series("t4_ppo_return", mean_ret, f"u{u}")
        print(f"T4PPO u{u}: ret={mean_ret:.2f} " +
        ...
        P.post_status({
            "training": {"status": "running", "name": "t4_ppo", "iter": u, "iters": UPDATES,
