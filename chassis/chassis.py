...
def build_chassis(p: ChassisParams) -> b.Part:
    ...
    for (mx, my) in p.motor_positions():
        ...
        # tapered arm from center to motor pad
        L = p.arm_length_mm
        ...
        pad = b.extrude(b.Circle(p.motor_pad_dia_mm/2).face(), p.body_thickness_mm)
        pad = pad.locate(b.Pos(L, 0, 0))
        piece = (arm + pad).rotate(b.Axis.Z, ang)
        arms.append(piece)
    body = arms[0]
    for a in arms[1:]:
        body = body + a
    ...
    for (mx, my) in p.motor_positions():
        ...
        for dx, dy in ((hs, hs), (-hs, hs), (-hs, -hs), (hs, -hs)):
            holes.append(b.Pos(mx+dx, my+dy, -1) * b.Cylinder(p.motor_hole_dia_mm/2, p.body_thickness_mm+2))
        holes.append(b.Pos(mx, my, -1) * b.Cylinder(p.motor_center_hole_dia_mm/2, p.body_thickness_mm+2))
    # stack holes (30.5 mm square) in hub
    sh = p.stack_spacing_mm / 2
    for dx, dy in ((sh, sh), (-sh, sh), (-sh, -sh), (sh, -sh)):
        holes.append(b.Pos(dx, dy, -1) * b.Cylinder(p.stack_hole_dia_mm/2, p.body_thickness_mm+2))
    for h in holes:
        body = body - h
