...
- CAMERA FOV (hard gate, user requirement): the cameras are the 2x Pi Camera 3 nose stereo pair ONLY (the belly pad-marker camera was REMOVED by user directive 2026-09-12 - downward sensing is lidar-only; remove the belly pocket/aperture and all pad_camera references from the design source). Both cameras mount lens-FORWARD (+X) and see out the nose apertures: no frame material inside the FOV pyramid (66.3h x 41.6v deg). chassis.py cuts the sight-line voids from components.camera_lens_poses() - keep that coupling; cameras in placement.json stay lens-forward at the nose.
...
placement.json may be edited to move components (the edit is shared by all three candidates; x/y/z in meters, z = component bottom) EXCEPT the pinned user-directive keys (vl53l9cx_breakout#n/ne/e/se/s/sw/w/nw/down, pi_camera_3#left/right) - components.py FIXED_PLACEMENT_KEYS ignores json overrides for those; do not waste mutations moving them.
