...
- PAD CAMERA (hard requirement, user directive 2026-09-09): a downward-facing pad-marker camera (placement.json key pad_camera, PINNED at belly center (0,0); 25x24x12 mm envelope, z=2mm component bottom) mounts in a recessed drop-in pocket in the belly floor at frame center (lateral offset <20mm by pin). The belly skin gets ONLY a minimal lens aperture at the mount; keep the lens face flush with or just proud of the belly underside so the camera's 65-degree half-angle downward cone (8 m range, pad-marker chain) passes NO frame material - the cone is steep (tan65 = 2.14), so any frame depth ahead of the lens widens the required opening fast. Do NOT grow the envelope for it; the pocket lives inside the existing floor/deck cavity. The diagonal ToF stations' ~44% world coverage is ACCEPTED BY DESIGN (2026-09-09 decision): they are arm/prop-line proximity monitors, not navigation sensors - do not spend mutations trying to clear the diagonal arms from their FoV.
- IMU LEVER ARM (gate): the IMU (ICM-42688-P, placement key "mpu9250" - legacy alias for icm42688p since 2026-09-12) must sit within 60 mm of the frame CoM (on/near the FC stack). The manifest exports its full transform (position + rotation + offset from CoM, schema 1.2) for the estimator.
...
placement.json may be edited to move components (the edit is shared by all three candidates; x/y/z in meters, z = component bottom) EXCEPT the pinned user-directive keys (vl53l9cx_breakout#n/ne/e/se/s/sw/w/nw, pi_camera_3#left/right, pad_camera) - components.py FIXED_PLACEMENT_KEYS ignores json overrides for those; do not waste mutations moving them.
