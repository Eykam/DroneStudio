...
- MASS (user directive 2026-09-06): the enclosure is TOO MASSIVE - printed mass is the primary optimization axis. Shave monocoque/shell/arm material everywhere the gates allow; no decorative fairing, no external pods, no excess wall beyond the 1.2mm dfam floor + FEA margins. The Pi Zero is REMOVED from the build (ee-flight's onboard compute replaces it - CM0 castellated SoM soldered onto the carrier PCBA, NOT a separate board) - do not add any bay/mount for it. The compute keep-out lives inside the fc_esc_stack PCBA envelope; that envelope stays at the contract dims until the EE CM0-carrier contract amendment lands - do not shrink it speculatively.
...
- BINDING CONSTRAINT (2026-09-13): the blocker right now is CRASH STRESS AT THE ARM ROOT, not mass. 17 straight generations failed the crash case marginally (17+ MPa vs the 16.5 MPa limit) while every other FEA case passed, and candidates drifted UP to 124-126 g doing it - the incumbent v163-g162b holds crash at 14.5 MPa at 120.4 g. Adding material without changing the load path is how the last 17 gens failed; mass creep with no stress win is a losing trade, not caution. Study how the incumbent carries the 3x motor-thrust crash load from pad to root to center (root section depth, fillet continuity, rib/web alignment, no thickness jumps at the root) and keep that load path intact. Win by reshaping the root section, not by uniform thickening: deeper root, smoother stress flow, then take the grams back elsewhere. A candidate over 120.4 g that still fails crash is strictly worse than doing nothing.
- MODAL TARGET (adoption constraint 2026-09-09): the 125 Hz first-mode soft floor now blocks every adoption - recent candidates read 121-122 Hz. Lift first mode above 125 Hz WITH MARGIN (aim >= 127 Hz) WHILE cutting mass: buy section inertia, not uniform thickness - deeper arm roots, vertical ribs and web depth, closed-section depth in the shell ring - and take the grams back from skin/canopy/wall areas. A candidate under 125 Hz cannot adopt however light it is.
- CAMERA FOV (hard gate, user requirement): the camera is a SINGLE Pi Camera 3, mono (user directive 2026-09-12: the stereo pair is DROPPED - the estimator consumes monocular only; the belly pad-marker camera was also REMOVED - downward sensing is lidar-only). Placement key pi_camera_3#main, PINNED lens-FORWARD (+X) at nose center (y=0) - merge the two old stereo apertures into ONE centered nose aperture sized for its FOV pyramid (66.3h x 41.6v deg): no frame material inside the pyramid. Remove all pi_camera_3#left / #right references and the stereo bay geometry from the design source. chassis.py cuts the sight-line voids from components.camera_lens_poses() - keep that coupling.
...
- DOWN TOF (hard requirement, user directive 2026-09-12): a 9th VL53L9CX (placement.json key vl53l9cx_breakout#down, PINNED at belly center (0,0), z=2mm component bottom) faces straight DOWN through a minimal aperture in the belly skin - this is the only downward sensor now (the pad camera is removed; downward sensing is lidar-only). Keep the sensor face flush with or just proud of the belly underside so its downward FoV cone passes NO frame material (HARD adoption gate: evaluate.py ray-traces the full 55x42deg pyramid from the aperture plane). The aperture replaces the old pad-camera pocket site; do NOT leave the old pocket geometry behind.
- CONTAINMENT (hard gate): every placed component (stack, battery, cameras, GPS, IMU) must sit FULLY INSIDE the frame envelope plus 2mm service clearance. The frame must keep an enclosed fuselage/deck cavity tall enough to swallow them OR recess bays; components.py placement() says where they sit (placement.json may be edited to move them - x/y/z in meters, z = component bottom). Enclose like DJI/Avata: battery and stack INSIDE the body, GPS in a rear fin/perch cavity, camera behind the nose shell with only the lens aperture forward.
...
- DESIGN LANGUAGE: read INSPIRATION.md in this directory first. The user wants the chassis to move beyond flat-plate freestyle frames toward DJI/Anduril-style form factors - faired enclosed body, swept/tapered arms with filleted roots, closed arm sections, recessed battery bay - while staying printable and serviceable.
- SPLIT-BODY PRINT (hard gate, user directive 2026-09-12): printers are Bambu P1S (256mm) / Creality Ender-3 V3 KE (220mm) - design to the 220x220mm bed. The monolithic ~260mm frame cannot fit (arm length is sim-contract-fixed, so no shrink-out). Split into 2-4 printable bodies - center body + bolt-on arms is the natural split; every printed piece must be <= 220x220mm in XY at flat-on-plate orientation (the bed_fit gate measures PER-BODY extents). Joints: M3 fasteners into printed bosses with alignment features (double-shear, >=2 fasteners per joint, boss walls >= 3mm); keep split planes off the arm-root max-bending section where the geometry allows; joints add ~2g per M3 boss pair and become crash-load paths.  HONEST LIMIT: the FEA sims the exported STEP as-is and does NOT model joint contact/fasteners - design joints conservatively; joint strength verification is a known follow-up.
- SPLIT DECLARATION (machine-checked): declare the split on ChassisParams as a list field, default empty (monolithic): split_planes = [{{"origin_mm": [x,y,z], "normal": [nx,ny,0], "n_bolts": 2, "boss_width_mm": W, "boss_height_mm": H, "bolt_grip_mm": T}}, ...]. The design still exports ONE solid (FEA/sim unchanged - the split is declarative, print-prep slices later). The evaluator slices the mesh at your declared planes for bed_fit (each piece <= 220x220) and runs joint_stress: section loads at each plane from the FEA load cases (crash 3x, cartwheel 2x lateral, torsion 1.5x - arm splits are statically determinate from the motor-pad loads) checked as bolt shear + boss bearing + boss bending vs PETG allowables (16.5 MPa). Planes must be vertical and sit inboard of a motor on that arm - the screen FAILS CLOSED otherwise. Declare REAL boss section dims at the joint; undersized joints fail.
...
placement.json may be edited to move components (the edit is shared by all three candidates; x/y/z in meters, z = component bottom) EXCEPT the pinned user-directive keys (vl53l9cx_breakout#n/ne/e/se/s/sw/w/nw/down, pi_camera_3#main) - components.py FIXED_PLACEMENT_KEYS ignores json overrides for those; do not waste mutations moving them.
...
def run_generation():
    ...
    ordered = sorted(results.values(), key=lambda c: (c["all_pass"], c["score"], -(c["mass_g"] or 1e9)), reverse=True)
    best_all_pass = st.get("best_all_pass", False)
    best_mass = st.get("best_mass_g")

    def _metric(v, *path):
        try:
            cur = json.load(open(os.path.join(HERE, "snapshots", v, "metrics.json")))
            for k in path:
    ...
    def _hold_reason(c):
        # parent standing rule 2026-09-07: quality-over-grams - stop adopting
        # mass-reduction candidates that erode the crash stress margin once < 1.0 MPa
        try:
            inc_s = _metric(st["best_variant"], "metrics", "fea", "crash", "max_von_mises_mpa")
            cand_s = _metric(c["variant"], "metrics", "fea", "crash", "max_von_mises_mpa")
        ...
        cand_s2 = _metric(c["variant"], "metrics", "fea", "crash", "max_von_mises_mpa")
        if (cand_s2 is not None and 16.5 - cand_s2 < 1.0
                and c["mass_g"] and best_mass and c["mass_g"] < best_mass):
            return ("QUALITY HOLD: %s own crash stress margin %.2f MPa < 1.0; mass-reduction adoption blocked"
                    % (c["variant"], 16.5 - cand_s2))
        # parent standing rule 2026-09-06: crash displacement hard floor 0.3mm margin (limit 5.0)
        cand_d = _metric(c["variant"], "metrics", "fea", "crash", "max_disp_mm")
        if cand_d is not None and cand_d > 4.70:
            return ("DISP FLOOR HOLD: %s crash disp %s mm breaches the 0.3mm margin floor (limit 5.0); adoption blocked"
                    % (c["variant"], cand_d))
        # parent directive 2026-09-09: cartwheel displacement watch - hold + surface (trend 4.57->4.63->4.72 across adopted bests)
        cand_cw = _metric(c["variant"], "metrics", "fea", "cartwheel", "max_disp_mm")
        if cand_cw is not None and cand_cw > 4.70:
            return ("CARTWHEEL DISP WATCH HOLD: %s cartwheel disp %s mm over the 4.70 watch line; adoption blocked, surface to parent"
                    % (c["variant"], cand_cw))
        # parent ruling 2026-09-08: modal soft floor 125 Hz (gate floor 120)
        cand_m = _metric(c["variant"], "metrics", "fea", "modal", "first_mode_hz")
        if cand_m is not None and cand_m < 125.0:
            return ("MODAL FLOOR HOLD: %s first mode %s Hz below the 125 Hz soft floor (gate 120); adoption blocked"
                    % (c["variant"], cand_m))
        return None

    def _improves(c):
        return (c["all_pass"] and not best_all_pass) or \
               (c["all_pass"] == best_all_pass and (c["score"] > st["best_score"] or
                (c["score"] == st["best_score"] and c["mass_g"] and (best_mass is None or c["mass_g"] < best_mass))))

    improved = False
    w = ordered[0]
    for cand in ordered:
        if not _improves(cand):
            continue
        hold = _hold_reason(cand)
        if hold:
            print(f"[gen {gen}] {hold}; trying next candidate", flush=True)
            continue
        improved = True
        w = cand
        break
    if not improved:
        c0 = ordered[0]
        print(f"[gen {gen}] no adoption: best candidate {c0['variant']} (all_pass={c0['all_pass']}, score={c0['score']:.3f}, mass={c0['mass_g']}) "
              f"vs incumbent {st['best_variant']} (score={st['best_score']:.3f}, mass={best_mass}, all_pass={best_all_pass})", flush=True)
